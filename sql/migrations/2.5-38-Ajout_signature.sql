...
CREATE TABLE `signature` (
  `ID_SIGNATURE` int(11) unsigned NOT NULL AUTO_INCREMENT,
  `DATE_SIGNATURE` datetime DEFAULT NULL,
  `ID_UTILISATEUR` bigint(20) unsigned NOT NULL,
  `ID_PIECEJOINTE` bigint(20) NOT NULL,  
  CONSTRAINT `fk_signature_utilisateur` FOREIGN KEY (`ID_UTILISATEUR`) REFERENCES `utilisateur` (`ID_UTILISATEUR`) ON DELETE CASCADE ON UPDATE CASCADE,
  CONSTRAINT `fk_signature_piecejointe` FOREIGN KEY (`ID_PIECEJOINTE`) REFERENCES `piecejointe` (`ID_PIECEJOINTE`) ON DELETE CASCADE ON UPDATE CASCADE,
